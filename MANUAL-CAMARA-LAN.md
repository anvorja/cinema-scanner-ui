...
`http://<tu-ip-local>:8090/scanner/` — por ejemplo `http://192.168.1.5:8090/scanner/`.
...
HTTP (`http://192.168.1.5:8090/scanner/`) no cuenta como contexto seguro, así que el
...
4. Toca **"Reiniciar"** en el banner azul de abajo (reinicia Chrome, no el
   teléfono).
5. Vuelve a abrir `http://192.168.1.5:8090/scanner/`, loguéate de nuevo y entra al
...
## Sin flags: el túnel de ngrok (HTTPS)

El scanner también se publica en `https://<dominio-ngrok>/scanner/` (ver
`../infra-cinema/COMPARTIR-CON-NGROK.md`, sección "Scanner de boletas"). Como
es HTTPS, la cámara funciona en cualquier celular y navegador sin tocar
flags, y desde cualquier red.

## Otras opciones (no usadas, mencionadas por si hace falta más adelante)

- **Túnel HTTPS con `cloudflared`** (`cloudflared tunnel --url
  http://localhost:8090`): da una URL `https://*.trycloudflare.com` real que
